...
The schema tool allows you to define your own structured data types. These types are composed of the predefined WasmLib value data types. The tool generates a struct with named fields according to the definition in the schema definition file, and also generates code to serialize and deserialize the structure to a byte array, so that it can be saved as a single unit of data.

You can use structs as a type in state storage definitions and the schema tool will automatically generate the proxy code to access it.

Say, you create a betting smart contract that needs to store information for each bet. The `Bet` structure consists of the bet amount and time, the number that we bet on, and the agent ID of the one who placed the bet. The smart contract keeps track of all bets in state storage in an array of `Bet` structs. For this smart contract, you need to modify the schema definition file as follows:
...
```json
{
  "structs": {
    "Bet": {
      "amount": "Int64 // The bet amount.",
      "time": "Int64 // The timestamp of the bet.",
      "number": "Int32 // The number that the user had bet on.",
      "better": "AgentID // The user who has placed this bet."
    }
  },
  "state": {
...
type Bet struct {
    ...
    Number int32             // number that we bet on
...
`toBytes()` serialization code to automatically transform byte arrays into `Bet` structs.
...
The end result is `ImmutableBettingState` and `MutableBettingState` that provide a type-safe interface to the state of the betting contract.