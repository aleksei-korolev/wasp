...
Since we allow nesting of [container types](../wasm_vm/proxies.mdx#container-proxies), it could become difficult to create proper declarations for such nested types, given that you can only indicate a single type, an array of single type, or a map of single type in a field definition.

To solve this problem we have introduced the `typedefs` section for schema definitions. There you can define a type name for a container which you could use in function and variable definitions and within other containers.

Extending the example from [the previous section](structs.mdx), imagine a similar smart contract that also needs to keep track of all betting rounds. Each `BettingRound` would contain an array of `Bet` values, something that we can define in the `typedefs` section:
...
Notice how the `rounds()` member function returns a proxy to an array of `BettingRound`, which in turn contains arrays of `Bet`. You cannot interpret an array of betting rounds as an array of bets on accident, as your access to every level within this hierarchy is checked during the compile time.